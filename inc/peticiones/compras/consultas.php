...
{           //recibe los datos correctamente
    try {
        ...
        //3.  REGISTRAR LOS DATOS DE PRODUCTO Y LA COMPRA EN DETALLE_COMPRA Y MANDAR MENSAJE DE EXITO
                $sql =  "INSERT INTO `detalle_pedido`(`id_usuario`, `id_producto`, `nombre_producto`,  `id_proveedor`, `cantidad`, `precio_venta`, `importe`) 
                        VALUES ('$id_usuario', '$id_producto', '$nombre_producto','$proveedor','$cantidad','$precio_venta','$importe')";
                $consulta = mysqli_query($conexion, $sql);
...
function mostrar_detalle(): array
{
    try {
        ...
        $sql = "SELECT `detalle_pedido`.`id_detalle_pedido`,`detalle_pedido`.`id_pedido`, 
                ...
                `detalle_pedido`.`nombre_producto` as producto, `detalle_pedido`.`id_proveedor`, 
